...
public class SqliteTaskDAO implements ITaskDAO {
    ...
    public void addTask(Task task) {
        try {
            PreparedStatement statement = connection.prepareStatement("INSERT INTO tasks (Title, startDate, endDate, description, tags, userId) VALUES (?, ?, ?, ?, ?, ?)");
    ...
    public void updateTask(Task task) {
        try {
            PreparedStatement statement = connection.prepareStatement(
                    "UPDATE tasks SET title = ?, startDate = ?, endDate = ?, description = ?, tags = ?, userId = ? WHERE id = ?");
            statement.setString(1, task.getTitle());
            statement.setString(2, task.getStartDate().toString());
            statement.setString(3, task.getEndDate().toString());
            statement.setString(4, task.getDescription());
